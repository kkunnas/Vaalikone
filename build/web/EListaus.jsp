    ...
    <body>
        <div id="container">
            ...
            <form method="POST"></br>
                ...
                %>
                <div id="muokkaaminen">
                    <b>Kysymys <%= i%>:</b> <%= kaikkiKysymykset.get(i - 1).getKysymys()%></br>
                    <b><small>Vastauksesi:</small></b>
                    <label>1</label><input type="radio" name="vastaus<%= i%>" value="1" <% if (vastaus == 1) {
                        out.println("checked='checked'");
                    }%>/>
                    <label>2</label><input type="radio" name="vastaus<%= i%>" value="2" <% if (vastaus == 2) {
                        out.println("checked='checked'");
                    }%>/>
                    <label>3</label><input type="radio" name="vastaus<%= i%>" value="3" <% if (vastaus == 3) {
                        out.println("checked='checked'");
                    }%>/>
                    <label>4</label><input type="radio" name="vastaus<%= i%>" value="4" <% if (vastaus == 4) {
                        out.println("checked='checked'");
                    }%>/>
                    <label>5</label><input type="radio" name="vastaus<%= i%>" value="5" <% if (vastaus == 5) {
                        out.println("checked='checked'");
                    }%>/></br>
                </div>
                <% }%>
